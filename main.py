...
def fk_to_board_frame(x_fk, y_fk, x_ref=None):
    ...
    if x_ref is not None:
        if x_ref < 0 and x_board > 0:
            x_board = -abs(x_board)
        elif x_ref > 0 and x_board < 0:
            x_board = abs(x_board)

    # 🔴 กัน Y กลับ เฉพาะตอน X < 0
    if x_ref is not None and x_ref < 0:
        y_board = -y_board
...
# ===================================================
    ...
    def board_cell_to_xy(self, r, c):
        # กระดาน 40x40 cm, 8x8
        cell_size = 40.0 / 8.0   # 5 cm

        # r=0 คือแถวบน → กลับแกน Y
        row_from_bottom = 7 - r

        x = BOARD_ORIGIN_X + (c + 0.5) * cell_size
        y = BOARD_ORIGIN_Y + (row_from_bottom + 0.5) * cell_size
